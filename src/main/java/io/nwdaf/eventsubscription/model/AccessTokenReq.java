...
public class AccessTokenReq {
	...
	@Pattern(regexp = "^([0-9A-Za-z]([-0-9A-Za-z]{0,61}[0-9A-Za-z])?\\.)+[A-Za-z]{2,63}\\.?$")
	@Size(min = 4, max = 253)
	 ...
	 * the following string \"set<Set ID>.sn<Service Name>.nfi<NF Instance
	 ...
	 * inserted at the left side to fill the 3 digits coding of MNC. Pattern:
	 * '^[0-9]{3}$' <NID> encoded as defined in clause 5.4.2 (\"Nid\" data type
	 * definition) <NFInstanceId> encoded as defined in clause 5.3.2 <ServiceName>
	...
	@Override
	public int hashCode() {
		return Objects.hash(grantType, nfInstanceId, nfType, targetNfType, scope, targetNfInstanceId, requesterPlmn,
				requesterPlmnList, requesterSnssaiList, requesterFqdn, requesterSnpnList, targetPlmn, targetSnpn,
				targetSnssaiList, targetNsiList, targetNfSetId, targetNfServiceSetId, hnrfAccessTokenUri,
				sourceNfInstanceId);
	}
